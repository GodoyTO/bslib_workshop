# Diagnostics Data Panel ####
dia_ui <- function(id) {nav_panel(
    ...
    layout_sidebar(
        sidebar = sidebar(
            ...
            width = 200,
            sliderInput(
                inputId = NS(id, "dia_sli"), label = "Select date of diagnosis:",
                min = min(data$dt_diag), max = max(data$dt_diag),
                value = range(data$dt_diag)
            ),
            sliderInput(
                inputId = NS(id, "age_sli"), label = "Select min and max age at diagnosis:",
                min = min(data$nu_age_diag), max = max(data$nu_age_diag),
                value = range(data$nu_age_diag)
            )
        ),
